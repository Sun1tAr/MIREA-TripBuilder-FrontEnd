...
// ===== Утилита конвертации waypoint для новой структуры времени и координат =====

const normalizeWaypoint = (wp) => {
    // Конвертируем широту и долготу в float
    ...
    // Если есть duration (старая структура), конвертируем в startDateTime и endDateTime
    if (wp.duration && !wp.startDateTime && !wp.endDateTime) {
        const startDateTime = wp.startDateTime || new Date().toISOString().slice(0, 16);
        const endDateTime = new Date(
            new Date(startDateTime).getTime() + wp.duration * 24 * 60 * 60 * 1000
        )
            .toISOString()
            .slice(0, 16);
        return {
